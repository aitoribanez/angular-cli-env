...
module.exports = {
  ...
  locals: function (options) {
    ...
    let field = Field.Field.prototype;

    const json = JSON.parse(fs.readFileSync(`${this.entityName}.json`, 'utf8'));
    this.quantity = json.entitys[0]['quantity'];
    let fields = json.entitys[0]['fields'];

    this.serverFields = field.makeFieldsObject(fields, this.entityName, this.entityNameCapitalize)
    console.log('serverRRR', this.serverFields);
    let serverFields = this.serverFields;
    this.serverFields = '{\n';
    for (let key in serverFields) {
      this.serverFields += `\t${key} : ${serverFields[key]},\n`
    }
    this.serverFields = this.serverFields.slice(0, -2);
    this.serverFields += '\n}';

    console.log('server', this.serverFields);
    // console.log('client', this.fields);
    ...
    return {
      ...
      // predefined
      entityName: this.entityName, // name introduced using command
      entityNameCapitalize: this.entityNameCapitalize, // name introduced using command capitalized
      fields: this.fields, // fields string needed for frontend
      serverFields: this.serverFields, // fields string needed for backend / server
      quantity: this.quantity // quantity elements to create on server
    };
  },
  files: function () {
